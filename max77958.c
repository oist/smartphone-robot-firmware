...
static int on_pd_msg_received(){
    synchronized_printf("Rec PD message\n");
...
static void on_ccstat_change(){
    synchronized_printf("ccstat changed\n");
    ...
    switch (CCStat){
	case 0b000:
	    synchronized_printf("ccstat changed to no connection\n");
	    vbus_turn_off();
	    break;
	case 0b001:
	    synchronized_printf("ccstat changed to SINK\n");
	    vbus_turn_off();
	    break;
	case 0b010:
	    synchronized_printf("ccstat changed to SOURCE\n");
	    //vbus_turn_on();
	    break;
	default: 
	    synchronized_printf("ccstat changed to %d\n", CCStat);
	    break;
	}
}

static void on_chgtype_change(){
    synchronized_printf("chgtype changed\n");
    memset(send_buf, 0, sizeof send_buf);
    memset(return_buf, 0, sizeof return_buf);
    ...
    switch (ChgType){
	case 0b000:
	    synchronized_printf("ChgTyp changed to nothing attached\n");
	    break;
	case 0b001:
	    synchronized_printf("ChgTyp changed to SDP, USB cable attached\n");
	    break;
	case 0b010:
	    synchronized_printf("ChgTyp changed to CDP, Charging Downstream Port\n");
	    break;
	default: 
	    synchronized_printf("ChgTyp changed to DCP, Dedicated charger\n");
	    break;
	}
}
...
static void opcode_queue_add(int32_t (opcode_func)(), int32_t opcode_data){
	...
	synchronized_printf("opcode_queue is full");
	assert(false);
    }
    synchronized_printf("Added to opcode_queue. %d entries remaining\n", queue_get_level(&opcode_queue));
} 
...
static int32_t parse_interrupt_vals(){
	...
	synchronized_printf("Power source ready\n");
...
static void on_ccistat_change(){
    synchronized_printf("CCIStat changed\n");
}

static void on_ccpinstat_change(){
    synchronized_printf("CCPinStat changed\n");
}
...
static void get_interrupt_vals(){
    ...
    synchronized_printf("interrupts vals: 0x4: 0x%02x, 0x5: 0x%02x, 0x6: 0x%02x, 0x7: 0x%02x\n", return_buf[0], return_buf[1], return_buf[2], return_buf[3]);
}
...
static int opcode_write(uint8_t *buf){
    ...
    if (buf[0] != 0x21){
	synchronized_printf("buffer should always start with the 0x21 register");
	assert(false);
    }

    i2c_write_error_handling(i2c0, MAX77958_SLAVE_P1, buf, sizeof(send_buf), false);
    //synchronized_printf("opcode_write: 0x%02x 0x%02x 0x%02x 0x%02x\n", buf[0], buf[1], buf[2], buf[3]);
...
static void opcode_read(){
    ...
    synchronized_printf("opcode_read: 0x%02x 0x%02x 0x%02x 0x%02x\n", return_buf[0], return_buf[1], return_buf[2], return_buf[3]);
...
void test_max77958_get_id(){
    synchronized_printf("test_max77958_get_id started...\n");
	...
	synchronized_printf("DEVICE_ID should be 0x58");
	assert(false);
    }
    if (return_buf[1] != 0x02){
	synchronized_printf("DEVICE_REV should be 0x02");
	assert(false);
    }
    synchronized_printf("test_max77958_get_id PASSED: DEVICE_ID = %x\n", return_buf[0]);
    synchronized_printf("test_max77958_get_id PASSED: DEVICE_REV = %x\n", return_buf[1]);
}

void test_max77958_get_customer_config_id(){
    synchronized_printf("test_max77958_get_customer_config_id started...\n");
	...
	i++;
	if (i > 10){
	    synchronized_printf("Error: Timed out waiting for GPIO to finish\n");
	    assert(false);
	}
    }
    if (return_buf[0] != 0x55){
	synchronized_printf("OPCODE should be 0x55");
	assert(false);
    }
    if ((return_buf[2] | (return_buf[3] << 8)) != 0x0B6A){
        synchronized_printf("CUSTOMER_CONFIG_ID should be 0x0B6A");	
	assert(false);
    }
    if ((return_buf[4] | (return_buf[5] << 8)) != 0x6860){
	synchronized_printf("CUSTOMER_CONFIG_REV should be 0x6860");
	assert(false);
    }
    synchronized_printf("test_max77958_get_customer_config_id: 0x51=0x%02x\n", return_buf[0]);
    synchronized_printf("test_max77958_get_customer_config_id: 0x52=0x%02x\n", return_buf[1]);
    synchronized_printf("test_max77958_get_customer_config_id: 0x53=0x%02x\n", return_buf[2]);
    synchronized_printf("test_max77958_get_customer_config_id: 0x54=0x%02x\n", return_buf[3]);
    synchronized_printf("test_max77958_get_customer_config_id: 0x55=0x%02x\n", return_buf[4]);
    synchronized_printf("test_max77958_get_customer_config_id: 0x56=0x%02x\n", return_buf[5]);
    synchronized_printf("test_max77958_get_customer_config_id: 0x57=0x%02x\n", return_buf[6]);
    synchronized_printf("test_max77958_get_customer_config_id: 0x58=0x%02x\n", return_buf[7]);
    synchronized_printf("test_max77958_get_customer_config_id: 0x59=0x%02x\n", return_buf[8]);
    synchronized_printf("test_max77958_get_customer_config_id: 0x5A=0x%02x\n", return_buf[9]);
    synchronized_printf("test_max77958_get_customer_config_id: 0x5B=0x%02x\n", return_buf[10]);

    synchronized_printf("test_max77958_get_customer_config_id PASSED: CUSTOMER_CONFIG_ID = 0x%04x\n", (return_buf[2] | (return_buf[3] << 8)));
    synchronized_printf("test_max77958_get_customer_config_id PASSED: CUSTOMER_CONFIG_REV = 0x%04x\n", (return_buf[4] | (return_buf[5] << 8)));
}

void test_max77958_interrupt(){
    synchronized_printf("test_max77958_interrupt started...\n");
    set_interrupt_masks_all_masked();

    test_max77958_started = true;
    synchronized_printf("test_max77958_interrupt: prior to driving low GPIO%d. Current Value:%d\n", _gpio_interrupt, gpio_get(_gpio_interrupt));
    gpio_set_dir(_gpio_interrupt, GPIO_OUT);
    if (gpio_get(_gpio_interrupt) != 0){
	synchronized_printf("test_max77958_interrupt: GPIO%d was not driven low. Current Value:%d\n", _gpio_interrupt, gpio_get(_gpio_interrupt));
	assert(false);
    }
    synchronized_printf("test_max77958_interrupt: after driving low GPIO%d. Current Value:%d\n", _gpio_interrupt, gpio_get(_gpio_interrupt));
    uint32_t i = 0;
	...
	i++;
	if (i > 1000){
	    synchronized_printf("test_max77958_interrupt timed out\n");
	    assert(false);
	}
    }
    ...
    synchronized_printf("test_max77958_interrupt: PASSED after %" PRIu32 " milliseconds.\n", i*10);
}
...
void max77958_init(uint gpio_interrupt, queue_t* cq, queue_t* rq){
    ...
    opcode_queue_pop();
    synchronized_printf("max77958 init finished\n");
}
...
static bool opcode_queue_pop(){
	...
	synchronized_printf("Removed entry from opcode_queue. %d entries remaining\n", queue_get_level(&opcode_queue));
	// if the call_queue is full, assert
	if(queue_try_add(call_queue_ptr, &entry)){
	    synchronized_printf("added opcode entry to call_queue\n");
	    return true;
	}else{
	    synchronized_printf("opcode_queue_pop: call_queue full\n");
	    assert(false);
	}
    }
    // if there is no entry in the opcode_queue, return false
    else {
	synchronized_printf("opcode_queue_pop: opcode_queue empty\n");
    	return false;
    }
}
...
static int32_t pd_msg_response(){
    ...
    i2c_write_error_handling(i2c0, MAX77958_SLAVE_P1, send_buf, 1, true);
    i2c_read_error_handling(i2c0, MAX77958_SLAVE_P1, return_buf, 1, false);
    synchronized_printf("PD_STATUS0: 0x%02x\n", return_buf[0]);
    switch (return_buf[0]){
        case PDMSG_PRSWAP_SRCTOSWAP:
	    synchronized_printf("PD Message: PRSWAP_SRCTOSWAP\n");
	    break;
	case PDMSG_PRSWAP_SWAPTOSNK:
	    synchronized_printf("PD Message: PRSWAP_SWAPTOSNK\n");
	    vbus_turn_off();
	    break;
	case PDMSG_PRSWAP_SNKTOSWAP:
	    synchronized_printf("PD Message: PRSWAP_SNKTOSWAP\n");
	    break;
	case PDMSG_PRSWAP_SWAPTOSRC:
	    synchronized_printf("PD Message: PRSWAP_SWAPTOSRC\n");
	    vbus_turn_on();
	    break;
	case PDMSG_VDM_NAK_RECEIVED:
	    synchronized_printf("PD Message: VDM_NAK Received\n");
	case PDMSG_VDM_BUSY_RECEIVED:
	    synchronized_printf("PD Message: VDM_BUSY_RECEIVED\n");
	case PDMSG_VDM_ACK_RECEIVED:
	    synchronized_printf("PD Message: VDM_ACK_RECEIVED\n");
	case PDMSG_VDM_REQ_RECEIVED:
	    synchronized_printf("PD Message: VDM_REQ_RECEIVED\n");
	    break;
	default:
	    synchronized_printf("PD Message: Unknown\n");
	    break;
	}	
    return 0;
}
